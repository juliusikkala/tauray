...
public:
     ...
     */
    static MemoryRegionPtr register_ptr(
        ProtectionDomainPtr pd,
        void* addr,
        size_t length,
        Access access = Access::LocalRead | Access::ZeroBased
     ...
     * @param access Bitfield of access qualifiers
     */
    static MemoryRegionPtr register_dmabuf(
        ProtectionDomainPtr pd,
        uint64_t offset,
        size_t length,
        int fd,
...
public:
     ...
     */
    static WorkRequest AtomicCompareAndSwap(
        uint64_t wr_id,
        const std::vector<ScatterGatherEntry>& sg_list,
        uint64_t remote_addr,
        uint32_t rkey,
...
public:
    ...
    static RdmaConnection
    connect(const char* address, uint16_t port = DEFAULT_PORT);
