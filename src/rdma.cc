...
WorkRequest::WorkRequest(const WorkRequest& other):
    wr{other.wr},
    mr_list{other.mr_list},
    ibv_sg_list{other.ibv_sg_list},
    next{}
{
...
void WorkRequest::initWithSgList(
    uint64_t id,
    ibv_wr_opcode op,
...
WorkRequest WorkRequest::RdmaRead(
    uint64_t wr_id,
    const std::vector<ScatterGatherEntry>& sg_list,
    uint64_t remote_addr,
    uint32_t rkey,
...
RdmaConnection::RdmaConnection(rdmacm::IdPtr cm_id):
    cm_id(cm_id),
    pd(ibverbs::ProtectionDomain::create(cm_id)),
